...
def repeats(our_str):
    """
    Проверяет сколько раз каждый символ встречался в строке, и добавляет количество к символам с помощью постфикса формата _n
    :param our_str: исходная строка
    :return: новая строка, в которой каждому символу добавлен постфикс формата _n, где n число повторений символа в строке
    """
    letters_dict = {}
    new_str = ''
    for symbol in our_str:
        if symbol not in letters_dict:
            ...
            letters_dict[symbol] += 1
            new_str += symbol + '_' + str(letters_dict[symbol])
    return new_str
