...
def everything_for_your_cat(cats_data):
    """
    Объединяет повторяющиеся записи по имени и фамилии покупателя
    :param cats_data: кортеж: Кличка котика, Возраст котика, Имя покупателя, Фамилия покупателя
    :return: строка: Имя_покупателя Фамилия_покупателя: Кличка_котика1, Возраст; ...; Кличка_котикаN, Возраст\n
    """
    customers = {}
    for row in cats_data:
        cat_name, cat_age, customer_name, customer_surname = row
        if (customer_name, customer_surname) not in customers:
            customers[(customer_name, customer_surname)] = [(cat_name, cat_age)]
        else:
            customers[(customer_name, customer_surname)].append((cat_name, cat_age))
    our_str = ""
    for owner, pets in customers.items():
        pets_str = "; ".join([f"{pet[0]}, {pet[1]}" for pet in pets])
        our_str += f"{owner[0]} {owner[1]}: {pets_str}\n"
    return our_str
