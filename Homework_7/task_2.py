...
class PersonInfo:
    def __init__(self, full_name, age, *departments):
        ...
        :param full_name: имя и фамилия сотрудника
        :param age: возраст
    ...
    def short_name(self):
        """
        Приводит имя и фимилию к виду "Фамилия И."
        """
    ...
    def path_deps(self):
        """
        Возвращает путь до подразделения сотрудника разделенный "-->"
        """
        return " --> ".join(self.departments)

    def new_salary(self):
        """
        Возвращает значение новой зарпалаты по формуле:
        1337*Возраст*суммарное кол-во вхождений трех наиболее часто встречающихся букв из списка подразделений
        (регистр имеет значение "А" и "а" - разные буквы)
        """
        dep_count = {}
        ...
        top3 = sorted(dep_count, key=dep_count.get, reverse=True)[:3]
        top_count = sum([dep_count[x] for x in top3])
        return 1337 * self.age * top_count
